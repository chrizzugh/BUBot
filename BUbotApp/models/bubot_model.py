#!Preprocessing of the input text for the model
...
with open('/content/drive/MyDrive/TRAINING FILES AND OUTPUT/INPUTS/abbrev.json', 'r') as abbreviations:
    abbrev = json.load(abbreviations)

with open('/content/drive/MyDrive/TRAINING FILES AND OUTPUT/OUTPUTS/vocab.json', 'r') as vocabulary:
...
savedmodel = tf.keras.models.load_model('/content/drive/MyDrive/TRAINING FILES AND OUTPUT/VanilaBUbot')

model.set_weights(savedmodel.get_weights())

tf.keras.utils.plot_model(model, to_file='/content/drive/MyDrive/TRAINING FILES AND OUTPUT/Photos/version1.png',show_layer_names=True, show_shapes=True)
